...
function() {
  ...
  self.initialize = function($scope, $element/*, $ionicHistory*/) {
    self.selectedIndex = function () {
      return self.tabs.indexOf(selectedTab.get());
    };
    ...
    self.deselect = function (tab) {
      if (tab.$tabSelected) {
        previousSelectedTab = selectedTab.get();
    ...
    self.select = function (tab, shouldEmitEvent) {
      ...
      // todo: deal with this.
      if (false && selectedTab.get() && selectedTab.get().$historyId == tab.$historyId) {
        if (shouldEmitEvent) {
        ...
        if (shouldEmitEvent) {
          $scope.trigger('$ionicHistory.change', {
            type: 'tab',
            tabIndex: tabIndex,
