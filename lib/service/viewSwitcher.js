 ...
 * TODO document
 */
meteoric.service.ionicViewSwitcher =
function() {
  let $document = meteoric.Utils.to$element(document),
      $q = Promise,
      $ionicClickBlock = null,  // todo: take care of this.
      $ionicConfig = meteoric.service.ionicConfig,
      $ionicNavBarDelegate = meteoric.service.ionicNavBarDelegate;
      ...
      var switcher = {
        ...
        loadViewElements: function(registerData) {
          ...
          if (!alreadyInDom) {
            ...
            // meteoric: If there are two, then the other one is leaving.
            console.log(viewElements.length);
            leavingEle = viewElements.length > 1 ? viewElements.eq(0) : undefined;
          }
        ...
        render: function(registerData, callback) {
          if (alreadyInDom) {
            ...
            // Note: We never cache so no point.
            ...
            navViewCtrl.scope.trigger('$ionicView.loaded', enteringData);
          }
        ...
        transition: function(direction, enableBack, allowAnimate) {
          ...
          function onReflow() {
            ...
            // todo: trigger auto transitions on the associated nav bars
          ...
          function transitionComplete() {
            ...
            // todo : tell the nav bars that the transition has ended
            /*$ionicNavBarDelegate._instances.forEach(function(instance) {
        ...
        emit: function(step, enteringData, leavingData) {
          ...
          if (step == 'after') {
            if (enteringScope) {
              enteringScope.trigger('$ionicView.enter', enteringData);
            }

            if (leavingScope) {
              leavingScope.trigger('$ionicView.leave', leavingData);

            } else if (enteringScope && leavingData && leavingData.viewId) {
              enteringScope.trigger('$ionicNavView.leave', leavingData);
            }
          }

          if (enteringScope) {
            enteringScope.trigger('$ionicView.' + step + 'Enter', enteringData);
          }

          if (leavingScope) {
            leavingScope.trigger('$ionicView.' + step + 'Leave', leavingData);

          } else if (enteringScope && leavingData && leavingData.viewId) {
            enteringScope.trigger('$ionicNavView.' + step + 'Leave', leavingData);
          }
        },
        ...
        cleanup: function(transData) {
          ...
          if (leavingEle && transData.direction == 'back' &&
  ...
  function getViewElementIdentifier(locals, view) {
    ...
    // Note: stateId is, afaik, angular-ui-router related. Although iron-router
    //       is a similar state machine. I'll stick with this one for now.
  ...
  function destroyViewEle(ele) {
    ...
    if (ele && ele.length) {
      ...
      if (viewScope) {
        viewScope.trigger('$ionicView.unloaded', ele.data(DATA_VIEW));
        viewScope.$destroy();
      }
      ele.remove();
    }
