...
let ionicHistory =
function() {
  let $rootScope = meteoric.Utils.$rootScope,
      $state = new meteoric.IronRouterAdapter(Router),
      $location = meteoric.location,
      $window = window,
  ...
  function getParentHistoryObj(scope) {
    console.log(scope, scope.hasOwnProperty);
  ...
  function getCurrentStateId() {
    var id;
    if ($state && $state.current() && $state.routeName()) {
      id = $state.routeName();
      if ($state.params()) {
  ...
  return {
    ...
    register: function(parentScope, viewLocals) {
      ...
      if (forcedNav) {
        ...
        viewHistory.views[viewId] = this.createView({
          ...
          url: url,
          canSwipeBack: true //canSwipeBack(ele, viewLocals)  // todo: deal with this
        });
    ...
    currentStateName: function() {
      return ($state && $state.current() ? $state.routeName() : null);
    },
...
meteoric.service.ionicHistory = new ionicHistory();
ionicHistoryRun(
    meteoric.Utils.$rootScope,
    new meteoric.IronRouterAdapter(Router),
    meteoric.location,
    document,
    meteoric.service.ionicPlatform,
    meteoric.service.ionicHistory,
    meteoric.service.IONIC_BACK_PRIORITY
);
function ionicHistoryRun (
    $rootScope, $state, $location, $document, $ionicPlatform, $ionicHistory, IONIC_BACK_PRIORITY) {
