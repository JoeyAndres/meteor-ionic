...
function($scope, $element, $attrs) {
  let $controller = null, // todo: meant to fail, this have no use to us since we can't cache anyway.
      $compile = null,  // todo: meant to fail
      $ionicNavBarDelegate = meteoric.service.ionicNavBarDelegate,
      $ionicNavViewDelegate = meteoric.service.ionicNavViewDelegate,
      $ionicHistory = meteoric.service.ionicHistory,
      $ionicViewSwitcher = meteoric.service.ionicViewSwitcher,
      $ionicConfig = meteoric.service.ionicConfig,
      $ionicScrollDelegate = meteoric.service.ionicScrollDelegate;
  ...
  self.init = function() {
    ...
    var parent = $.inheritedData($element.parent()[0], '$uiView');
    ...
    $ionicNavViewDelegate.addAggregate(self);
    $scope.on('$destroy', function() {
      $ionicNavViewDelegate.removeAggregate(self);
  ...
  self.register = function(viewLocals) {
    ...
    registerData.ele = self.getViewElements().last();
  ...
  self.update = function(registerData) {
    ...
    var parentNavViewCtrl = $.inheritedData($element.parent()[0], '$ionNavViewController');
  ...
  self.transitionEnd = function() {
    ...
    for (x = 0, l = viewElements.length; x < l; x++) {
      ...
      if (navViewAttr(viewElement, VIEW_STATUS_ACTIVE) === 'entering' ||
          viewElement.data(DATA_ELE_IDENTIFIER) === activeEleId) {
        ...
        // If not active, then its destroyed.
        // $(elem).data is not working.
        $ionicViewSwitcher.destroyViewEle(viewElement);
      }
    }
  ...
  function onTabsLeave(ev, data) {
    ...
    for (var x = 0, l = viewElements.length; x < l; x++) {
      ...
      if (navViewAttr(viewElement) == VIEW_STATUS_ACTIVE) {
        ...
        viewScope && viewScope.trigger(ev.name.replace('Tabs', 'View'), data);
        break;
      }
    }
  }
  ...
  function getAssociatedNavBarCtrl() {
    ...
    return $.inheritedData($element[0], '$ionNavBarController');
  }
