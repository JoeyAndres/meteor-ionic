...
function($scope, $element, $attrs) {
  ...
  $ionicNavBarDelegate.addInstance(self);
  ...
  self.createHeaderBar = function(isActive) {
      var containerEle = jqLite($element.children(isActive ? '[nav-bar-block=1]' : '[nav-bar-block=2]'));
      ionic.DomUtil.cachedAttr(containerEle, 'nav-bar', isActive ? 'active' : 'cached');

      var alignTitle = $attrs.alignTitle || $ionicConfig.navBar.alignTitle();
      var headerBarEle = jqLite(containerEle.find('ion-header-bar')[0]);
      ...
      // todo: deal with cases when there is a custom onClick handler.
      navEle[BACK_BUTTON] && navEle[BACK_BUTTON].click(function() {
  ...
  self.update = function(viewData) {
    ...
    var enteringHeaderBar = self.isInitialized ?
        getOffScreenHeaderBar() : getOnScreenHeaderBar();
  ...
  $scope.$on('$destroy', function() {
    ...
    $ionicNavBarDelegate.removeInstance(self);
  });
